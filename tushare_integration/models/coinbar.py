...
class Coinbar(Base):
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'symbol': {'type': 'str', 'required': True, 'description': '数字货币交易对'}, 'start_date': {'type': 'datetime', 'required': False, 'description': '开始时间'}, 'end_date': {'type': 'datetime', 'required': False, 'description': '结束时间'}, 'exchange': {'type': 'str', 'required': True, 'description': '交易所名称'}, 'freq': {'type': 'str', 'required': True, 'description': '行情频率'}, 'contract_type': {'type': 'str', 'required': False, 'description': '合约类型'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    date = Column('date', DateTime, nullable=False, default="1970-01-01 00:00:00", server_default=text("'1970-01-01 00:00:00'"), comment='行情时间')
    ...
    count = Column('count', Integer, nullable=False, default=0, server_default=text("'0'"), comment='成交笔数(默认不展示，部分交易所无数据)')
    contract_type = Column('contract_type', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='合约类型')
