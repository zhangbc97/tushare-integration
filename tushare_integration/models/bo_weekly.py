...
class BoWeekly(Base):
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'date': {'type': 'str', 'required': False, 'description': '日期（每周一日期）'}, 'start_date': {'type': 'str', 'required': False, 'description': '开始日期'}, 'end_date': {'type': 'str', 'required': False, 'description': '结束日期'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    week_amount = Column('week_amount', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='当周票房(万)')
