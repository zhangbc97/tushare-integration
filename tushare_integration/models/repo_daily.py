...
class RepoDaily(Base):
    ...
    repo_maturity = Column(
        'repo_maturity', String(), nullable=False, default="", server_default=text("''"), comment='期限品种'
    )
    pre_close = Column(
        'pre_close', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='前收盘(%)'
    )
    ...
    weight_r = Column(
        'weight_r', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='加权价(利率债)(%)'
    )
    amount = Column(
