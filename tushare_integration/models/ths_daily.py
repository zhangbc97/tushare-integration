...
class ThsDaily(Base):
    ...
    __tablename__: str = 'ths_daily'
    __api_id__: ClassVar[int] = 260
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'ts_code': {'type': 'str', 'required': False, 'description': '指数代码'}, 'trade_date': {'type': 'str', 'required': False, 'description': '交易日期（YYYYMMDD格式，下同）'}, 'start_date': {'type': 'str', 'required': False, 'description': '开始日期'}, 'end_date': {'type': 'str', 'required': False, 'description': '结束日期'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    avg_price = Column('avg_price', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='平均点位')
