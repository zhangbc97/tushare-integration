...
class HkHold(Base):
    ...
    __api_path__: ClassVar[List[str]] = ['数据接口', '沪深股票', '特色数据', '沪深股通持股明细']
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'code': {'type': 'str', 'required': False, 'description': '交易所代码'}, 'ts_code': {'type': 'str', 'required': False, 'description': 'TS股票代码'}, 'trade_date': {'type': 'str', 'required': False, 'description': '交易日期'}, 'start_date': {'type': 'str', 'required': False, 'description': '开始日期'}, 'end_date': {'type': 'str', 'required': False, 'description': '结束日期'}, 'exchange': {'type': 'str', 'required': False, 'description': 'SH沪股通SZ深股通'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    exchange = Column('exchange', String(), nullable=False, default="", server_default=text("''"), comment='类型：SH沪股通SZ深港通')