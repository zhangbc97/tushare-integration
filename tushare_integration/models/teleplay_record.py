...
class TeleplayRecord(Base):
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'report_date': {'type': 'str', 'required': False, 'description': '备案月份'}, 'start_date': {'type': 'str', 'required': False, 'description': '备案开始月份（YYYYMM）'}, 'end_date': {'type': 'str', 'required': False, 'description': '备案结束月份（YYYYMM）'}, 'org': {'type': 'str', 'required': False, 'description': '备案机构'}, 'name': {'type': 'str', 'required': False, 'description': '电视剧名称'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    report_date = Column('report_date', Date, nullable=False, default="1970-01-01", server_default=text("'1970-01-01'"), comment='报备时间')
    license_key = Column('license_key', String(), nullable=False, default="", server_default=text("''"), comment='许可证号')
    episodes = Column('episodes', String(), nullable=False, default="", server_default=text("''"), comment='集数')
    shooting_date = Column('shooting_date', Date, nullable=False, default="1970-01-01", server_default=text("'1970-01-01'"), comment='拍摄时间')
    prod_cycle = Column('prod_cycle', String(), nullable=False, default="", server_default=text("''"), comment='制作周期')
    content = Column('content', String(), nullable=False, default="", server_default=text("''"), comment='内容提要')
    pro_opi = Column('pro_opi', String(), nullable=False, default="", server_default=text("''"), comment='省级管理部门备案意见')
    dept_opi = Column('dept_opi', String(), nullable=False, default="", server_default=text("''"), comment='相关部门意见')
