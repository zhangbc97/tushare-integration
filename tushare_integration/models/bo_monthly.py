...
class BoMonthly(Base):
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'date': {'type': 'str', 'required': True, 'description': '日期（每月1号）'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    list_date = Column('list_date', Date, nullable=False, default="1970-01-01", server_default=text("'1970-01-01'"), comment='上映日期')
    avg_price = Column('avg_price', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='平均票价')
    month_amount = Column('month_amount', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='当月票房(万)')
