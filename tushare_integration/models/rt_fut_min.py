...
class RtFutMin(Base):
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'topic': {'type': 'str', 'required': False, 'description': '主题'}, 'freq': {'type': 'str', 'required': True, 'description': '分钟类型'}, 'ts_code': {'type': 'str', 'required': True, 'description': '证券代码'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    trade_date = Column('trade_date', Date, nullable=False, default="1970-01-01", server_default=text("'1970-01-01'"), comment='结算日')
    action_date = Column('action_date', Date, nullable=False, default="1970-01-01", server_default=text("'1970-01-01'"), comment='交易日')