...
class KplConcept(Base):
    """题材数据(开盘啦)"""

    __tablename__: str = 'kpl_concept'
    __api_id__: ClassVar[int] = 350
    __api_name__: ClassVar[str] = 'kpl_concept'
    __api_title__: ClassVar[str] = '题材数据(开盘啦)'
    __api_info_title__: ClassVar[str] = '开盘啦题材库'
    __api_path__: ClassVar[List[str]] = ['数据接口', '沪深股票', '打板专题数据', '题材数据（开盘啦）']
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'trade_date': {'type': 'str', 'required': False, 'description': '交易日期'}, 'ts_code': {'type': 'str', 'required': False, 'description': '题材代码(xxxxxx.KP格式)'}, 'name': {'type': 'str', 'required': False, 'description': '题材名称'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
