...
class SwDaily(Base):
    ...
    __api_path__: ClassVar[List[str]] = ['数据接口', '指数', '申万行业指数日行情']
    ...
    __api_params__: ClassVar[Dict[str, Any]] = {'ts_code': {'type': 'str', 'required': False, 'description': '行业代码'}, 'trade_date': {'type': 'str', 'required': False, 'description': '交易日期'}, 'start_date': {'type': 'str', 'required': False, 'description': '开始日期'}, 'end_date': {'type': 'str', 'required': False, 'description': '结束日期'}, 'limit': {'type': 'int', 'required': False, 'description': '单次返回数据长度'}, 'offset': {'type': 'int', 'required': False, 'description': '请求数据的开始位移量'}}
    ...
    float_mv = Column('float_mv', Float, nullable=False, default=0.0, server_default=text("'0.0'"), comment='流通市值(万元)')
