...
本地缓存是为了优化数据写入性能而设计的功能。大多数OLAP数据库（如ClickHouse、Doris等）都推荐使用大批量写入的方式导入数据，尤其在存算分离的架构下，小批量频繁写入会导致性能显著下降。通过配置本地缓存，系统可以先将数据暂存在本地（内存或磁盘），当累积到一定数量后再批量写入远程数据库，从而提高整体性能和稳定性。
